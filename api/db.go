...
func GetAGITree(all bool) (agi []AGI, err error) {
	...
	if all {
		sqlStr = `WITH RECURSIVE subordinates AS (
				...
				) SELECT id, parent_id, repo_name, repo_full_name, repo_owner, repo_html_url, repo_description, repo_created_at, repo_pushed_at, repo_homepage, repo_size, repo_forks_count, repo_stargazers_count, repo_subscribers_count, repo_open_issues_count, repo_license_name, repo_license_spdx_id, repo_license_url, repo, category, name, description, homepage, category_html_id, depth FROM subordinates order by path, repo_stargazers_count`
	}
	stmt, _ := db.Prepare(sqlStr)
	defer stmt.Close()
	rows, err := stmt.Query()
